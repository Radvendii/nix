...
{
    if (v.type(state->values) == nPath) {
        auto storePath = fetchToStore(state->fetchSettings, *state->store, v.path(state->values), FetchMode::Copy);
        return {{
            .path =
                DerivedPath::Opaque{
    ...
    else if (v.type(state->values) == nString) {
        return {{
