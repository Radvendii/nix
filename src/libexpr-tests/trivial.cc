...
TEST_F(TrivialExpressionTest, updateAttrs)
{
    ...
    ASSERT_NE(a, nullptr);
    ASSERT_THAT(a->value.toStack(state.values), IsIntEq(3));

    auto b = v.attrs()->find(createSymbol("b"));
    ASSERT_NE(b, nullptr);
    ASSERT_THAT(b->value.toStack(state.values), IsIntEq(2));
}
...
TEST_P(AttrSetMergeTrvialExpressionTest, attrsetMergeLazy)
{
    ...
    ASSERT_THAT(a->value.toStack(state.values), IsAttrsOfSize(2));

    auto b = a->value.attrs(state.values)->find(createSymbol("b"));
    ASSERT_NE(b, nullptr);
    ASSERT_THAT(b->value.toStack(state.values), IsIntEq(1));

    auto c = a->value.attrs(state.values)->find(createSymbol("c"));
    ASSERT_NE(c, nullptr);
    ASSERT_THAT(c->value.toStack(state.values), IsIntEq(2));
}
