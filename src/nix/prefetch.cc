...
{
    {
        ...
        if (!fromExpr) {
            ...
            /* Extract the URL. */
            auto * attr = v.attrs(state->values)->get(state->symbols.create("urls"));
            if (!attr)
            ...
            auto attr2 = v.attrs(state->values)->get(state->symbols.create("outputHashMode"));
            if (!attr2)
            ...
            /* Extract the name. */
            if (!name) {
                auto attr3 = v.attrs(state->values)->get(state->symbols.create("name"));
                if (!attr3)
                    name =
