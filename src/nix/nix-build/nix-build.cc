...
static void main_nix_build(int argc, char ** argv)
{
    ...
    if (isNixShell) {
        ...
        newArgs.alloc("inNixShell").mkBool(state->values, true);
        for (auto & i : *autoArgs)
            newArgs.insert(i);
    ...
    for (auto e : exprs) {
        ...
        takesNixShellAttr = [&](const ValueRef v) {
            ...
            bool add = false;
            if (v.type(state->values) == nFunction && v.lambda(state->values).fun.hasFormals(state->exprs)) {
                for (auto & i : v.lambda(state->values).fun.payload(state->exprs).formals->formals) {
