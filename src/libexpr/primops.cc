...
void prim_exec(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    auto elems = args[0].listView(state.values);
    auto count = args[0].listSize(state.values);
    if (count == 0)
...
static void prim_typeOf(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    case nFunction:
        ...
        break;
    case nExternal:
        t = args[0].external(state.values)->typeOf();
        break;
    case nFloat:
        t = "float";
        break;
    ...
    bool operator()(ValueRef v1, ValueRef v2, std::string_view errorCtx) const
    {
        try {
            if (v1.type(state.values) == nFloat && v2.type(state.values) == nInt)
                return v1.fpoint(state.values) < v2.integer(state.values).value;
            if (v1.type(state.values) == nInt && v2.type(state.values) == nFloat)
                return v1.integer(state.values).value < v2.fpoint(state.values);
...
#pragma GCC diagnostic ignored "-Wswitch-enum"
            ...
            case nInt:
                return v1.integer(state.values) < v2.integer(state.values);
            case nFloat:
                return v1.fpoint(state.values) < v2.fpoint(state.values);
            case nString:
                return strcmp(v1.c_str(state.values), v2.c_str(state.values)) < 0;
            case nPath:
                // Note: we don't take the accessor into account
                // since it's not obvious how to compare them in a
                // reproducible way.
                return strcmp(v1.pathStr(state.values), v2.pathStr(state.values)) < 0;
            case nList:
                // Lexicographic comparison
                for (size_t i = 0;; i++) {
                    if (i == v2.listSize(state.values)) {
                        return false;
                    } else if (i == v1.listSize(state.values)) {
                        return true;
                    } else if (!state.eqValues(v1.listView(state.values)[i], v2.listView(state.values)[i], pos, errorCtx)) {
                        return (*this)(v1.listView(state.values)[i], v2.listView(state.values)[i], "while comparing two list elements");
                    }
                }
            default:
                state
...
static void prim_genericClosure(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    auto startSet = state.getAttr(
        state.sStartSet, args[0].attrs(state.values), "in the attrset passed as argument to builtins.genericClosure");
    ...
    ValueList workSet;
    for (auto elem : startSet->value.listView(state.values))
    ...
    auto op = state.getAttr(
        state.sOperator, args[0].attrs(state.values), "in the attrset passed as argument to builtins.genericClosure");
    state.forceFunction(
    ...
    if (ceilValue >= int_min && ceilValue < -int_min) {
        ...
        state
            .error<EvalError>(
                ...
                args[0].integer(state.values).value)
            .atPos(pos)
            .debugThrow();
    } else {
        state.error<EvalError>("NixFloat argument %1% is not in the range of NixInt", args[0].fpoint(state.values))
            .atPos(pos)
            .debugThrow();
    }
    // `forceFloat` casts NixInt to NixFloat, but instead NixInt args shall be returned unmodified
    if (isInt) {
        auto arg = args[0].integer(state.values);
        auto res = v.integer(state.values);
        if (arg != res) {
            state
                .error<EvalError>(
...
static void prim_trace(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    if (args[0].type(state.values) == nString)
        printError("trace: %1%", args[0].string_view(state.values));
    else
...
static void prim_pathExists(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    try {
        ...
        auto mustBeDir =
            arg.type(state.values) == nString && (arg.string_view(state.values).ends_with("/") || arg.string_view(state.values).ends_with("/."));
...
static void prim_dirOf(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    if (args[0].type(state.values) == nPath) {
        auto path = args[0].path(state.values);
...
static void prim_findFile(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    for (auto v2 : args[0].listView(state.values)) {
        state.forceAttrs(v2, pos, "while evaluating an element of the list passed to builtins.findFile");

        std::string prefix;
        auto i = v2.attrs(state.values)->find(state.sPrefix);
        if (i != v2.attrs(state.values)->end())
            prefix = state.forceStringNoCtx(
                i->value,
                pos,
                "while evaluating the `prefix` attribute of an element of the list passed to builtins.findFile");

        i = state.getAttr(state.sPath, v2.attrs(state.values), "in an element of the __nixPath");
...
static void prim_attrValues(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    for (size_t n = 0; n < args[0].attrs(state.values)->size(); n++)
        list[n] = std::bit_cast<ValueRef>((uint32_t)n);

    std::sort(list.begin(), list.end(), [&](ValueRef v1, ValueRef v2) {
        std::string_view s1 = state.symbols[(*args[0].attrs(state.values))[std::bit_cast<uint32_t>(v1)].name];
        std::string_view s2 = state.symbols[(*args[0].attrs(state.values))[std::bit_cast<uint32_t>(v2)].name];
        return s1 < s2;
    });

    for (auto & v : list)
        v = (*args[0].attrs(state.values))[std::bit_cast<uint32_t>(v)].value;
...
void prim_getAttr(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    auto i = state.getAttr(state.symbols.create(attr), args[1].attrs(state.values), "in the attribute set under consideration");
    // !!! add to stack trace?
...
static void prim_unsafeGetAttrPos(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    auto i = args[1].attrs(state.values)->find(state.symbols.create(attr));
    if (i == args[1].attrs(state.values)->end())
...
void prim_lineOfPos(EvalState & state, PosIdx pos, ValueRef * args, ValueRef v) {
    v.mkInt(state.values, state.positions[PosIdx(args[0].integer(state.values).value)].line);
}
void prim_columnOfPos(EvalState & state, PosIdx pos, ValueRef * args, ValueRef v) {
    v.mkInt(state.values, state.positions[PosIdx(args[0].integer(state.values).value)].column);
}
...
static void prim_hasAttr(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    v.mkBool(state.values, args[1].attrs(state.values)->find(state.symbols.create(attr)) != args[1].attrs(state.values)->end());
}
...
static void prim_removeAttrs(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    names.reserve(args[1].listSize(state.values));
    for (auto elem : args[1].listView(state.values)) {
        ...
        names.emplace_back(state.symbols.create(elem.string_view(state.values)), ValueRef::null);
    }
    ...
    /* Copy all attributes not in that set.  Note that we don't need
       ...
       vector. */
    auto attrs = state.buildBindings(args[0].attrs(state.values)->size());
    std::set_difference(
        args[0].attrs(state.values)->begin(), args[0].attrs(state.values)->end(), names.begin(), names.end(), std::back_inserter(attrs));
...
static void prim_listToAttrs(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    auto listView = args[0].listView(state.values);
    ...
    for (const auto & [n, v2] : enumerate(listView)) {
        ...
        auto j = state.getAttr(state.sName, v2.attrs(state.values), "in a {name=...; value=...;} pair");
    ...
    for (size_t n = 0; n < listSize; n++) {
        ...
        auto j = state.getAttr(state.sValue, v2.attrs(state.values), "in a {name=...; value=...;} pair");
        prev = attr.name;
...
static void prim_intersectAttrs(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    auto & left = *args[0].attrs(state.values);
    auto & right = *args[1].attrs(state.values);
...
static void prim_catAttrs(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    SmallValueVector<nonRecursiveStackReservation> res(args[1].listSize(state.values));
    size_t found = 0;

    for (auto v2 : args[1].listView(state.values)) {
        state.forceAttrs(
            v2, pos, "while evaluating an element in the list passed as second argument to builtins.catAttrs");
        if (auto i = v2.attrs(state.values)->get(attrName))
            res[found++] = i->value;
    }
...
static void prim_mapAttrs(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    auto attrs = state.buildBindings(args[1].attrs(state.values)->size());
...
static void prim_zipAttrsWith(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    const auto listItems = args[1].listView(state.values);
    ...
    for (auto & vElem : listItems) {
        ...
        for (auto & attr : *vElem.attrs(state.values))
...
static void prim_elemAt(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    if (n < 0 || std::make_unsigned_t<NixInt::Inner>(n) >= args[0].listSize(state.values))
        state.error<EvalError>("'builtins.elemAt' called with index %d on a list of size %d", n, args[0].listSize(state.values))
            .atPos(pos)
            .debugThrow();
...
static void prim_tail(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    auto list = state.buildList(args[0].listSize(state.values) - 1);
    for (const auto & [n, v] : enumerate(list))
        v = args[0].listView(state.values)[n + 1];
    v.mkList(state.values, list);
}
...
static void prim_map(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    auto list = state.buildList(args[1].listSize(state.values));
    for (const auto & [n, v] : enumerate(list))
        (v = state.allocValue()).mkApp(state.values, args[0], args[1].listView(state.values)[n]);
    v.mkList(state.values, list);
}
...
static void prim_length(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    v.mkInt(state.values, args[0].listSize(state.values));
}
...
static void prim_foldlStrict(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    if (args[2].listSize(state.values)) {
        ValueRef vCur = args[1];

        auto listView = args[2].listView(state.values);
        for (auto [n, elem] : enumerate(listView)) {
            ValueRef vs[]{vCur, elem};
            vCur = n == args[2].listSize(state.values) - 1 ? v : state.allocValue();
...
static void prim_sort(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    auto list = state.buildList(len);
    for (const auto & [n, v] : enumerate(list))
        state.forceValue(v = args[1].listView(state.values)[n], pos);

    auto comparator = [&](ValueRef a, ValueRef b) {
        /* Optimization: if the comparator is lessThan, bypass
           callFunction. */
        if (args[0].isPrimOp(state.values)) {
            auto ptr = args[0].primOp(state.values)->fun.target<decltype(&prim_lessThan)>();
...
static void prim_concatMap(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    auto nrLists = args[1].listSize(state.values);
...
static void prim_substring(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    if (len == 0) {
        ...
        if (args[2].type(state.values) == nString) {
            v.mkString(state.values, "", args[2].context(state.values));
            return;
        }
    }
...
static void prim_convertHash(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    auto inputAttrs = args[0].attrs(state.values);
    ...
    auto iteratorToHashFormat = state.getAttr(
        state.symbols.create("toHashFormat"), args[0].attrs(state.values), "while locating the attribute 'toHashFormat'");
...
static void prim_concatStringsSep(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    std::string res;
    res.reserve((args[1].listSize(state.values) + 32) * sep.size());
    bool first = true;
...
static void prim_replaceStrings(EvalState & state, const PosIdx pos, ValueRef * args, ValueRef v)
{
    ...
    if (args[0].listSize(state.values) != args[1].listSize(state.values))
    ...
    from.reserve(args[0].listSize(state.values));
    for (auto elem : args[0].listView(state.values))
    ...
    auto to = args[1].listView(state.values);
...
void EvalState::createBaseEnv(const EvalSettings & evalSettings)
{
    ...
    const_cast<Bindings *>(getBuiltins().attrs(values))->sort();
