...
static void setDocPosition(ParserState * state, ExprRefOf<teLambda> lambda, PosIdx start) {
...
static ExprRef makeCall(Exprs & exprs, PosIdx pos, ExprRef fn, ExprRef arg) {
    if (auto e2 = fn.dyn_cast<teCall>()) {
        e2.payload(exprs).args.push_back(arg);
        return fn;
    }
    return exprs.add<teCall>(pos, fn, {arg});
}
...
%union {
  ...
  nix::ExprRef e;
  nix::ExprRefOf<nix::teList> list;
  nix::ExprRefOf<nix::teAttrs> attrs;
  nix::Formals * formals;
  nix::Formal * formal;
  nix::NixInt n;
  nix::NixFloat nf;
...
expr_function
  : ID ':' expr_function
    { auto me = state->exprs.add<teLambda>(CUR_POS, state->symbols.create($1), nullptr, $3);
      $$ = me;
      SET_DOC_POS(me, @1);
    }
  | formal_set ':' expr_function[body]
    { auto me = state->exprs.add<teLambda>(CUR_POS, state->validateFormals($formal_set), $body);
      $$ = me;
      SET_DOC_POS(me, @1);
    }
  | formal_set '@' ID ':' expr_function[body]
    {
      ...
      auto me = state->exprs.add<teLambda>(CUR_POS, arg, state->validateFormals($formal_set, CUR_POS, arg), $body);
      $$ = me;
      SET_DOC_POS(me, @1);
    }
  ...
  | ASSERT expr ';' expr_function
    { $$ = state->exprs.add<teAssert>(CUR_POS, $2, $4); }
  | WITH expr ';' expr_function
    { $$ = state->exprs.add<teWith>(CUR_POS, $2, $4); }
  | LET binds IN_KW expr_function
    { if (!$2.payload(state->exprs).dynamicAttrs.empty())
        throw ParseError({
            .msg = HintFmt("dynamic attributes not allowed in let"),
            .pos = state->positions[CUR_POS]
        });
      $$ = state->exprs.add<teLet>($2, $4);
    }
  | expr_if
  ;

expr_if
  : IF expr THEN expr ELSE expr { $$ = state->exprs.add<teIf>(CUR_POS, $2, $4, $6); }
  | expr_pipe_from
  | expr_pipe_into
  | expr_op
  ;
...
expr_op
  : '!' expr_op %prec NOT { $$ = state->exprs.add<teOpNot>($2); }
  | '-' expr_op %prec NEGATE { $$ = state->exprs.add<teCall>(CUR_POS, state->exprs.add<teVar>(state->s.sub), {state->exprs.add<teInt>(state->values, 0), $2}); }
  | expr_op EQ expr_op { $$ = state->exprs.add<teOpEq>($1, $3); }
  | expr_op NEQ expr_op { $$ = state->exprs.add<teOpNEq>($1, $3); }
  | expr_op '<' expr_op { $$ = state->exprs.add<teCall>(state->at(@2), state->exprs.add<teVar>(state->s.lessThan), {$1, $3}); }
  | expr_op LEQ expr_op { $$ = state->exprs.add<teOpNot>(state->exprs.add<teCall>(state->at(@2), state->exprs.add<teVar>(state->s.lessThan), {$3, $1})); }
  | expr_op '>' expr_op { $$ = state->exprs.add<teCall>(state->at(@2), state->exprs.add<teVar>(state->s.lessThan), {$3, $1}); }
  | expr_op GEQ expr_op { $$ = state->exprs.add<teOpNot>(state->exprs.add<teCall>(state->at(@2), state->exprs.add<teVar>(state->s.lessThan), {$1, $3})); }
  | expr_op AND expr_op { $$ = state->exprs.add<teOpAnd>(state->at(@2), $1, $3); }
  | expr_op OR expr_op { $$ = state->exprs.add<teOpOr>(state->at(@2), $1, $3); }
  | expr_op IMPL expr_op { $$ = state->exprs.add<teOpImpl>(state->at(@2), $1, $3); }
  | expr_op UPDATE expr_op { $$ = state->exprs.add<teOpUpdate>(state->at(@2), $1, $3); }
  | expr_op '?' attrpath { $$ = state->exprs.add<teOpHasAttr>($1, std::move(*$3)); delete $3; }
  | expr_op '+' expr_op
    { $$ = state->exprs.add<teConcatStrings>(state->at(@2), false, new std::vector<std::pair<PosIdx, ExprRef> >({{state->at(@1), $1}, {state->at(@3), $3}})); }
  | expr_op '-' expr_op { $$ = state->exprs.add<teCall>(state->at(@2), state->exprs.add<teVar>(state->s.sub), {$1, $3}); }
  | expr_op '*' expr_op { $$ = state->exprs.add<teCall>(state->at(@2), state->exprs.add<teVar>(state->s.mul), {$1, $3}); }
  | expr_op '/' expr_op { $$ = state->exprs.add<teCall>(state->at(@2), state->exprs.add<teVar>(state->s.div), {$1, $3}); }
  | expr_op CONCAT expr_op { $$ = state->exprs.add<teOpConcatLists>(state->at(@2), $1, $3); }
  | expr_app
  ;
...
expr_select
  : expr_simple '.' attrpath
    { $$ = state->exprs.add<teSelect>(CUR_POS, $1, std::move(*$3), ExprRef::null); delete $3; }
  | expr_simple '.' attrpath OR_KW expr_select
    { $$ = state->exprs.add<teSelect>(CUR_POS, $1, std::move(*$3), $5); delete $3; $5.warnIfCursedOr(state->exprs, state->symbols, state->positions); }
  | /* Backwards compatibility: because Nixpkgs has a function named ‘or’,
    ...
    expr_simple OR_KW
    { $$ = state->exprs.add<teCall>(CUR_POS, $1, {state->exprs.add<teVar>(CUR_POS, state->s.or_)}, state->positions.add(state->origin, @$.endOffset)); }
  | expr_simple
  ;
...
expr_simple
  : ID {
      ...
      if ($1.l == s.size() && strncmp($1.p, s.data(), s.size()) == 0)
          $$ = state->exprs.add<tePos>(CUR_POS);
      else
          $$ = state->exprs.add<teVar>(CUR_POS, state->symbols.create($1));
  }
  | INT_LIT { $$ = state->exprs.add<teInt>(state->values, $1); }
  | FLOAT_LIT { $$ = state->exprs.add<teFloat>(state->values, $1); }
  | '"' string_parts '"' { $$ = $2; }
  ...
  | path_start string_parts_interpolated PATH_END {
      ...
      $$ = state->exprs.add<teConcatStrings>(CUR_POS, false, $2);
  }
  | SPATH {
      std::string path($1.p + 1, $1.l - 2);
      $$ = state->exprs.add<teCall>(CUR_POS,
          state->exprs.add<teVar>(state->s.findFile),
          {state->exprs.add<teVar>(state->s.nixPath),
           state->exprs.add<teString>(state->values, std::move(path))});
  }
  | URI {
      ...
      $$ = state->exprs.add<teString>(state->values, std::string($1));
  }
  | '(' expr ')' { $$ = $2; }
  /* Let expressions `let {..., body = ...}' are just desugared
     into `(rec {..., body = ...}).body'. */
  | LET '{' binds '}'
    { $3.payload(state->exprs).recursive = true; $3.payload(state->exprs).pos = CUR_POS; $$ = state->exprs.add<teSelect>(noPos, $3, state->s.body); }
  | REC '{' binds '}'
    { $3.payload(state->exprs).recursive = true; $3.payload(state->exprs).pos = CUR_POS; $$ = $3; }
  | '{' binds1 '}'
    { $2.payload(state->exprs).pos = CUR_POS; $$ = $2; }
  | '{' '}'
    { $$ = state->exprs.add<teAttrs>(CUR_POS); }
  | '[' expr_list ']' { $$ = $2; }
  ;

string_parts
  : STR { $$ = state->exprs.add<teString>(state->values, std::string($1)); }
  | string_parts_interpolated { $$ = state->exprs.add<teConcatStrings>(CUR_POS, true, $1); }
  | { $$ = state->exprs.add<teString>(state->values, ""); }
  ;

string_parts_interpolated
  : string_parts_interpolated STR
  { $$ = $1; $1->emplace_back(state->at(@2), state->exprs.add<teString>(state->values, std::string($2))); }
  ...
  | STR DOLLAR_CURLY expr '}' {
      ...
      $$->emplace_back(state->at(@1), state->exprs.add<teString>(state->values, std::string($1)));
...
path_start
  : PATH {
    ...
    $$ =
        ...
        literal.front() == '/'
        ? state->exprs.add<tePath>(state->values, state->rootFS, std::move(path))
        : state->exprs.add<tePath>(state->values, state->basePath.accessor, std::move(path));
  }
  | HPATH {
    ...
    $$ = state->exprs.add<tePath>(state->values, ref<SourceAccessor>(state->rootFS), std::move(path));
  }
  ;
  ...
  | binds[accum] INHERIT attrs ';'
    { $$ = $accum;
      for (auto & [i, iPos] : *$attrs) {
          ...
          $accum.payload(state->exprs).attrs.emplace(
              i.symbol,
              AttrDef(state->exprs.add<teVar>(iPos, i.symbol), iPos, AttrDef::Kind::Inherited));
      }
      delete $attrs;
    }
    ...
    { $$ = $accum;
      ...
      for (auto & [i, iPos] : *$attrs) {
          ...
          $accum.payload(state->exprs).attrs.emplace(
              i.symbol,
              AttrDef(
                  state->exprs.add<teSelect>(iPos, from, i.symbol),
                  iPos,
...
expr_list
  : expr_list expr_select { $$ = $1; $1.payload(state->exprs).elems.push_back($2); /* !!! dangerous */; $2.warnIfCursedOr(state->exprs, state->symbols, state->positions); }
  | { $$ = state->exprs.add<teList>(); }
  ;
